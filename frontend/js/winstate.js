...
// Are we running under nodejs or nw
if (typeof require === 'function') {
   ...
   // This sets the body to black when the app is run without transparency, so the white
   // text shows up
   if (!win.isTransparent ()) {

      // pass the function you want to call at 'window.onload', in the function defined above
      var setBlackBackground = function () {
         // Set the background black
         document.body.style.background = 'black';

         // This might be needed if the window size seems to change!
         // win.resizeTo (216, 384);
      };

      var oldOnLoad = window.onload;
      if (typeof window.onload != 'function') {
         window.onload = setBlackBackground;
      } else {
   ...
   function restoreWindowState() {
       ...
       for (var i = 0; i < screens.length; i++) {
           ...
           if (winState.x > screen.bounds.x && winState.x < screen.bounds.x + screen.bounds.width) {
               if (winState.y > screen.bounds.y && winState.y < screen.bounds.y + screen.bounds.height) {
                   console.log("Location of window (" + winState.x + "," + winState.y + ") is on screen " + JSON.stringify(screen));
       ...
       if (!locationIsOnAScreen) {
           console.log("Last saved position of windows is not usable on current monitor setup. Moving window to center!");
